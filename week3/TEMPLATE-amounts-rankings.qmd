...
ggplot(heatmap_order, aes(x = year, 
  ...
  scale_x_continuous(breaks = seq(2010, 2020, by =2)) + # declutter year axis
  labs(fill = "Annual Mean Surface Water-Supply and\nUse Index (SUI)") + # use \n to break text string
  guides(fill = guide_colorbar(title.position = "top", # title to top
                               barwidth = 15,   # change color bar length
                               barheight = 0.75)) +
  theme_minimal() +
